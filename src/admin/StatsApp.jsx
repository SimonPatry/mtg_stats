    ...
    <>
      ...
      <aside className="side-nav" aria-label="Menu">
        ...
        {/* Sur écran étroit la déconnexion quitte la barre du haut, où elle
            grignotait le titre : elle atterrit ici, comme les exports. */}
        <div className="side-nav-section">
          <p className="side-nav-section-label">Session</p>
          <button type="button" className="side-nav-link" onClick={() => { onClose(); signOut() }}>
            Déconnexion
          </button>
        </div>
      </aside>
...
function StatsApp({ mode = 'admin', onBackToForge, embedded = false, initialView, onMemberViewChange }) {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([loadGames(), loadUsers(), loadDecks()])
  ...
  async function handleAddGame({ game, users: updatedUsers, decks: updatedDecks }) {
    // L'ordre compte : la partie référence des joueurs et des versions de deck
    // qui doivent exister en base avant elle, sinon la clé étrangère refuse.
    await saveUsers(updatedUsers, users)
    const { createdIds: createdDeckIds } = await saveDecks(updatedDecks, decks)

    const persisted = remapGameIds(game, createdDeckIds)
    await saveGames([...games, persisted], games)

    // On relit les trois collections : les identifiants qui font foi sont ceux
    // rendus par l'API, pas ceux que le formulaire a fabriqués localement.
  ...
  function handleCatalogSave({ users: updatedUsers, decks: updatedDecks }) {
    // PlayersManager a déjà persisté via l’API ; on met juste l’état à jour.
